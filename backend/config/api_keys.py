...
def _get_default_keys() -> APIKeys:
    ...
    configured = [name for name, status in keys.get_status().items() if status]
    if configured:
        logger.info(f"API keys loaded from environment: {', '.join(configured)}")
    else:
        logger.warning("No API keys configured. Set environment variables to enable data sources.")
